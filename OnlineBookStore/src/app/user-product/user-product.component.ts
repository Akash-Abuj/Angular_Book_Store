...
export class UserProductComponent {
  ...
  ngOnInit(){ // called by default as our web page loaded in browser
    this.userService.getProduct().subscribe(
      ...
      this.userService.getByEmail(this.userService.user_email).subscribe((response)=>{this.user=response; 
  ...
  addToCart(prodid:number,name:string,price:number){
    ...
    this.userService.totalsum=this.totalsum;
  }
 ...
 // cphone:number=91000100001;
  Placeholder(phone:number){
    if(this.cust.name != "" && this.cust.addrs!=""){
      if( this.userService.totalsum!=0){
  ...
  }else alert("Please Select a Book to Purchase.")  
}else alert("Fields Are Mandatory:")
  }
}
